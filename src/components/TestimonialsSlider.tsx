...
const GoogleIcon = () => <svg viewBox="0 0 24 24" className="w-3 h-3 flex-shrink-0">
...
const TestimonialsSlider = () => {
  ...
  return <div className="mb-2">
      <div ref={scrollRef} onScroll={handleScroll} className="flex gap-2 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-2" style={{
      scrollbarWidth: 'none',
      msOverflowStyle: 'none'
    }}>
        {testimonials.map((t, i) => <div key={i} className="snap-start flex-shrink-0 w-[75%] bg-card border border-border rounded-xl px-3 py-2.5">
            {/* Top row: stars + source */}
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-0.5">
                {Array.from({
              length: 5
            }).map((_, j) => <Star key={j} className="w-3 h-3 fill-warning text-warning" />)}
              </div>
            ...
            {/* Text */}
            <p className="text-foreground leading-relaxed mb-1.5 text-sm font-sans font-medium">"{t.text}"</p>
      ...
      <div className="flex items-center justify-center gap-1.5 mt-1">
        {testimonials.map((_, i) => <div key={i} className={`h-1 rounded-full transition-all ${i === activeIdx ? 'bg-primary w-3' : 'bg-muted-foreground/20 w-1.5'}`} />)}
      </div>
    </div>;
};
