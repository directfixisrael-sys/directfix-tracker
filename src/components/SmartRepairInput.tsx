...
interface SmartRepairInputProps {
  models: Array<{ id: string; name: string; original_screen_price: number; compatible_screen_price: number; battery_price: number }>;
  repairTypes: Array<{ id: string; name: string; is_phone_only: boolean }>;
...
const SmartRepairInput = ({ models, repairTypes, onModelAndRepairFound, onModelFound }: SmartRepairInputProps) => {
  ...
  const matchedModel = result?.model_name 
    ? models.find(m => m.name === result.model_name) 
    : null;

  const matchedRepairs = result?.repair_names
    ?.map(name => repairTypes.find(r => r.name === name))
    .filter(Boolean) || [];
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder='למשל: "מסך לאייפון 14" או "סוללה ל-15 פרו"'
              className="pr-10 text-right h-12 text-base rounded-xl"
              dir="rtl"
              disabled={isLoading}
            />
          </div>
        ...
        <Card className="p-4 animate-fade-in border-primary/20 bg-primary/5">
          <div className="space-y-3">
            ...
            {matchedRepairs.length > 0 ? (
              ...
              <Button
                variant="outline"
                className="w-full justify-center h-auto py-3 rounded-xl"
