        ...
        <div className="w-[calc(100%-2rem)] max-w-md bg-card rounded-2xl p-7 pb-8 shadow-2xl animate-scale-in border-2 border-primary/20">
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <Input
                ...
                type="tel"
                dir="ltr"
                className={`h-16 text-lg rounded-xl px-5 text-right transition-colors ${/^0(5\d{8}|[2-489]\d{7})$/.test(introPhone.replace(/\D/g, '')) ? 'border-2 border-success pl-12' : ''}`}
              />
              {/^0(5\d{8}|[2-489]\d{7})$/.test(introPhone.replace(/\D/g, '')) && (
          ...
          <label className="flex items-start gap-2.5 mt-3 cursor-pointer">
            ...
            <span className="text-[11px] text-muted-foreground leading-relaxed">
              אני מאשר/ת שקראתי והסכמתי ל<a href="https://directfix.co.il/privacy-policy/" target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-primary font-medium underline">מדיניות הפרטיות</a> ו<a href="/terms" target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-primary font-medium underline">תנאי השימוש</a>, ומאשר/ת יצירת קשר לתיאום התיקון
            </span>
          </label>
          ...
          {/* Brands marquee */}
          <div className="mt-6 -mx-7 -mb-2 pt-4 border-t border-border/40">
            <p className="text-center text-[10px] text-muted-foreground/70 mb-2 tracking-wider">תומכים בכל הדגמים</p>
            <div className="relative overflow-hidden" dir="ltr" style={{ maskImage: 'linear-gradient(to right, transparent, black 12%, black 88%, transparent)', WebkitMaskImage: 'linear-gradient(to right, transparent, black 12%, black 88%, transparent)' }}>
              <div className="flex gap-10 py-2 whitespace-nowrap animate-[marquee_22s_linear_infinite] w-max">
                {[...Array(2)].map((_, i) => (
